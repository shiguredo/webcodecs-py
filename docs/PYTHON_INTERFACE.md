...
- `NVIDIA_VIDEO_CODEC` - NVIDIA GPU（H.264/H.265/AV1、Linux/Windows のみ）
...
- Linux または Windows（macOS は非対応）
- ビルド時に `NVIDIA_VIDEO_CODEC=1` と `NVIDIA_VIDEO_CODEC_SDK_PATH` 環境変数が必要
...
| AV1 | o | o | NVENC / NVDEC* | Linux/Windows |
| H.264 | o | o | VideoToolbox* | macOS |
| H.264 | o | o | NVENC / NVDEC* | Linux/Windows |
| H.265 | o | o | VideoToolbox* | macOS |
| H.265 | o | o | NVENC / NVDEC* | Linux/Windows |

*ハードウェアアクセラレーション使用

**注**: NVIDIA Video Codec SDK (NVENC/NVDEC) を使用するには、ビルド時に `NVIDIA_VIDEO_CODEC=1` 環境変数を設定し、`NVIDIA_VIDEO_CODEC_SDK_PATH` で SDK のパスを指定する必要があります。
