...
int VideoDecoder::handle_display_picture(void* user_data, void* disp_info_ptr) {
  ...
  // VideoFrame を作成
  VideoFrameBufferInit buffer_init;
  buffer_init.format = VideoPixelFormat::NV12;
  buffer_init.timestamp = disp_info->timestamp;
  buffer_init.coded_width = width;
  buffer_init.coded_height = height;
  buffer_init.planes = {
      VideoFramePlaneInit{frame_data.data(), static_cast<uint32_t>(width)},
      VideoFramePlaneInit{frame_data.data() + y_size,
                          static_cast<uint32_t>(width)}};

  auto video_frame = std::make_unique<VideoFrame>(buffer_init);
...
bool VideoDecoder::decode_nvdec(const EncodedVideoChunk& chunk) {
  ...
  packet.payload = chunk.data().data();
  packet.payload_size = static_cast<unsigned long>(chunk.data().size());
...
#endif  // defined(NVIDIA_VIDEO_CODEC)
